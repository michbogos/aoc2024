    ...
    while node != end:
        for dx in range(-21, 21):
            for dy in range(-21, 21):
                if node[1]+dx > -1 and node[1]+dx < len(mat[0]) and node[0]+dy > -1 and node[0]+dy < len(mat) and (node[0]+dy,node[1]+dx) in path and abs(dx)+abs(dy) <= 20:
                    if mat[node[0]+dy][node[1]+dx] != "#" and (dist[node[0]+dy][node[1]+dx])-dist[node[0]][node[1]]-(abs(dx)+abs(dy)-1) >= 100 and dist[node[0]+dy][node[1]+dx] < 10e9:
                        res += 1
        # for dy in [3,-3]:
